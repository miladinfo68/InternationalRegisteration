...
namespace ISR.Commons
{
    public class EncryptionClass
    {
        ...
        static public String DecryptRJ256(byte[] cypher)
        {
            var sRet = "";
            ...
            byte[] KeyString = Encoding.ASCII.GetBytes(key);
            byte[] IvString = Encoding.ASCII.GetBytes(IV);
            //var Key = KeyString;
            //var IV = IvString;
            ...
            using (var rj = new RijndaelManaged())
            {
                ...
                using (var cs = new CryptoStream(ms, rj.CreateDecryptor(KeyString, IvString), CryptoStreamMode.Read))
                {
                    using (var sr = new StreamReader(cs))
                    {
                        sRet = sr.ReadLine();
        ...
        //RJ256
        public static string EncryptRJ(string prm_text_to_encrypt, string prm_key, string prm_iv)
        {
            ...
            //var toEncrypt= Encoding.GetEncoding(437).GetBytes(sToEncrypt);
        ...
        public static string EncryptRJ(string prm_text_to_encrypt)
        {
            string key = "b3afc5fd20e3637160da4f9cab6c8072";
            string IV = "a214ee38a470c5974c10498b7152ca39";
            ...
            //
            var encryptor = rj.CreateEncryptor(KeyString, IvString);
